...
public class LatheSpline : MeshCreator {
	...
	public override void RecalculateMesh() {
		...
		for (int curveIndex = 1; curveIndex<=curveCount-1; curveIndex++) { //start at 1, because we need to access spline at splineIndex-1
        ...
        { //start at 1, because we need to access vertex at vertexIndex-1
            ...
            //generate 4 vertices (quad):
            int v0 = getIndex(NumCurves - 1, vertexIndex - 1, vertexCount);
            int v1 = getIndex(0, vertexIndex - 1, vertexCount);
            int v2 = getIndex(0, vertexIndex, vertexCount);
            int v3 = getIndex(NumCurves - 1, vertexIndex, vertexCount);
