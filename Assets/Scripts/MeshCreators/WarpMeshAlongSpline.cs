...
public class WarpMeshAlongSpline : MeshCreator
{
	...
	public override void RecalculateMesh() {
		...
		for (int i = 0; i < points.Count-1; i++) {
			...
			if (i == 0)
			{
				quaternion = Quaternion.LookRotation(lineSegmentDirection, Vector3.up);
			...
			else
			{
				...
				direction += lineSegmentDirection.normalized; 
				direction.Normalize();

				quaternion = Quaternion.LookRotation(direction, Vector3.up);

			}
			localOrientation.Add(quaternion);
		}
		...
		for (int i = 0; i < points.Count-2; i++) {
			...
			int subMeshesAmount = InputMesh.subMeshCount;
            for (int k = 0; k < subMeshesAmount; k++)
            {
				var tries = InputMesh.GetTriangles(k); 
				for (int j = 0; j < tries.Length; j += 3)
				{
					builder.AddTriangle(
						tries[j] + numVerts * i,
						tries[j + 1] + numVerts * i,
						tries[j + 2] + numVerts * i,
						k
					);
				}
			}
