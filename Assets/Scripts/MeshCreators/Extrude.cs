...
public class Extrude : MeshCreator {
	...
	public override void RecalculateMesh() {
		...
		for (int i = 0; i < points.Count; i++) {
			...
			int v2 = builder.AddVertex(new Vector3(points[j].x, points[j].y, 0), new Vector2(1, 0));
			// back vertices:
			int v3 = builder.AddVertex(new Vector3(points[i].x, points[i].y, height), new Vector2(1, 1));
			int v4 = builder.AddVertex(new Vector3(points[j].x, points[j].y, height), new Vector2(0, 1));
			// Add quad:
	...
	void TriangulatePolygon(List<int> triangles, List<Vector2> polygon, List<int> indices) {
		for (int i = 0; i < polygon.Count; i++) {
            ...
            {
				if (u == polygon[j] || v == polygon[j] || w == polygon[j]) continue;
                if (InsideTriangle(u, v, w, polygon[j])) skip = true;
            }
            if (skip) continue;
